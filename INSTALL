...
after return impl->path;

add this just before the function closes: 
    #endif

OR MERGE the files included in my fork (vstguiFilestoReplace) with the vstgui inside the vst3sdk/vstgui4/ directory
You can do it either manually or with command: cp -r  <dir1>/* <dir2>
...
***Release works for me only if i use clang: premake5 --cc=clang gmake)
